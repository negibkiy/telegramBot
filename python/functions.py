...
def choice_osn_podrazdeleniya(choice_podrazdelenie, message):        # функция для выбора основных подразделений
    ...
    if choice_podrazdelenie == '📫 Деканат ФЭВТ':    # если выбрана "📫 Деканат ФЭВТ"
        ...
        if message.text == 'Какие виды стипендий бывают?':
            info = 'Всего существует два вида государственных стипендий:\n\n  1️⃣ Академическая,\n\n  2️⃣ Социальная.\n\n ⁉️ Однако, для особо заинтересованных студентов бывают еще и \"именные\" стипендии Президента РФ, Правительства РФ и иные стипендии. *(полный список можно уточнить в профкоме)\n\n ❗ Помимо этого есть еще и \"Социальная поддержка\"'
        if message.text == 'Как получить академическую стипендию?':
            info = 'Государственная академическая стипендия может быть назначена студентам ТОЛЬКО по результатам сессии.\n\nУсловия получения академической стипендии:\n\n ❗ cтудент должен обучаться  только за счет федерального бюджета (бесплатно),\n\n ❗ студент должен вовремя закрыть сессию (!без пересдач) на оценки: \" 5️⃣ отлично\" и \" 4️⃣ хорошо\".\n\n ‼️ всем студентам 1 курса, зачисленным на бюджетные места в 1 семестре выплачивается академическая стипендия,\n\n ‼️ Размер стипендии в следующем семестре (после закрытия первого семестра и других последующих - БЕЗ ПЕРЕСДАЧ) будет зависеть от суммы баллов по всем предметам \"ЗАКРЫТОЙ\" текущей сессии!\n *(зависимость среднего балла от размера академической стипендии можно посмотреть в этом чат-боте во вкладке: 🎓Основные подразделения ➡️ 📫Деканат ФЭВТ ➡️ Рейтинговая оценка системы знаний)'
        if message.text == 'Кто имеет право на социальную стипендию?':
        ...
        if message.text == 'Как получить социальную стипендию?':
            info = 'Право получения государственной социальной стипендии имеет студент:\n\n ❗ обучающийся на бюджетной основе (бесплатно) по очной форме обучения,\n\n ❗ подавший справку в деканат факультета (методисту) для получения государственной социальной стипендии, выдаваемую органом социальной защиты населения по месту жительства.\n\n ‼️ чтобы получить \"справку для получения государственной социальной стипендии\" необходимо собрать справки: о составе семьи; о доходах всех членов семьи за последние 3 месяца.\n Эта справка представляется ежегодно, служит основанием для издания приказа и хранится в институте. Приём справок осуществляется до 5 числа каждого месяца!'
        if message.text == 'Как получить социальную поддержку?':
            info = ' ❗ Социальная поддержка оказывается остро нуждающимся студентам на основании представленных заместителю декана факультета заявления и документов, подтверждающих необходимость в социальной поддержке.\n\n ❗ Социальная поддержка может выплачиваться только студентам дневного отделения, обучающимся за счет средств государственного бюджета (бесплатно).\n\n ❗ Студентам же, обучающимся за счет частичной или полной компенсации средств (платная форма обучения) выдача материальной помощи НЕ ПРОИЗВОДИТСЯ!'
